...
`enrich` deterministically validates `.descuff/llm-discovery.json` against evidence IDs, source fingerprints, input artifact hashes, relative source paths, and safety gates, then writes a reviewable LLM discovery diff. If no LLM discovery file exists, it still validates legacy `.descuff/semantic-enrichment.json` and writes a semantic diff. `validate` writes structured readiness explanations so tools can distinguish complete categories, acceptable gaps, recommendations, and blockers. When browser-agent scenarios exist, readiness explanations link score losses to affected scenarios and evidence surfaces.
