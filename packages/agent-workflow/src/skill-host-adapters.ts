...
export function renderSkillHostInstructions(input: RenderSkillHostInstructionsInput): string {
  ...
  const lines = [
    ...
    "## Intake",
    "",
    "Before running the full workflow, briefly explain that Descuff improves local Next.js apps by adding and validating agent-facing standards while preserving human-facing behavior.",
    ...
    "Ask for any missing inputs needed to run safely:",
    "",
    ...
    "",
    "If the user already provided enough information, proceed without asking again. If they decline hosted recon, do the local workflow only.",
    "",
    "## Workflow",
    "",
...
export function renderCodexSkillFile(packageName = "descuff"): string {
  return [
    ...
    "## Codex-Specific Loop",
    "",
    "1. Start with a short explanation and intake unless the user already supplied the needed choices.",
    ...
    "9. Report baseline, enrichment result, files changed, final validation, optional scenario/recon results, and remaining blockers.",
    "",
