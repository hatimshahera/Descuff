...
export function renderSharedSkillCoreInstructions(packageName = "descuff"): string {
  return [
    ...
    "Use the host agent only to propose evidence-backed LLM discovery and semantic enrichment.",
...
export function renderSkillHostInstructions(input: RenderSkillHostInstructionsInput): string {
  ...
  const lines = [
    ...
    "## Intake",
    "",
    "Before running any Descuff command, briefly explain that Descuff improves local supported apps by adding and validating agent-facing standards while preserving human-facing behavior.",
    "State that the current public preview supports local Next.js codebases and React/Vite preview codebases; hosted URL recon is optional and does not replace source-backed validation.",
    ...
    "2. Read `.descuff/skill-evidence-packet.json` and `.descuff/llm-discovery-prompt.md`.",
    "3. Produce LLM discovery using `.descuff/llm-discovery-template.json` and valid evidence IDs, then write it to `.descuff/llm-discovery.json`.",
    "4. If `.descuff/llm-discovery-template.json` is unavailable, fall back to `.descuff/semantic-enrichment-template.json` and `.descuff/semantic-enrichment.json`.",
    `5. Run \`npx ${packageName} enrich .\` and inspect the generated discovery or enrichment diff before implementation.`,
    "6. Implement only accepted plan items and preserve UI behavior unless explicitly approved.",
    `7. Run \`npx ${packageName} finish .\` and report the before/after result.`,
    `8. If scenario suggestions were requested, run \`npx ${packageName} scenarios .\` and inspect \`.descuff/scenario-suggestions.md\`.`,
    `9. If the user provided a hosted URL, run \`npx ${packageName} recon <hosted-url> --browser\` after implementation and report hosted browser-agent scenario results honestly.`,
    "",
    "## Final Report",
    "",
...
export function renderCodexSkillFile(packageName = "descuff"): string {
  return [
    "---",
    "name: descuff",
    'description: "Use when a developer asks Codex to Descuff a local supported app: run Descuff analysis, perform evidence-backed semantic enrichment, implement accepted agent-facing standards, and validate the before/after result."',
    "---",
    "",
    "# Descuff",
    "",
    "Use this skill when the user asks to run Descuff, make a local supported app more usable by AI agents, or explicitly invokes `$descuff`.",
    "",
    ...
    "3. Read `.descuff/skill-evidence-packet.json` and `.descuff/llm-discovery-prompt.md`.",
    "4. Write strict JSON LLM discovery to `.descuff/llm-discovery.json` when semantic discovery is enabled. Fall back to `.descuff/semantic-enrichment.json` only when the LLM discovery template is unavailable.",
    `5. Run \`npx ${packageName} enrich .\` and inspect the generated discovery or enrichment diff.`,
