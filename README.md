...
- `check` performs the same deterministic, non-LLM drift analysis, fast-passes unrelated changes, writes a validation plan, and runs validation for changes that can affect routes, APIs, capabilities, auth boundaries, or published standards. Passing checks say no Descuff repair is needed; failing checks point the coding agent back to the `start -> finish -> check` repair workflow.
